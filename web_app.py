...
def process_question_route():
    """
    RECEBE a pergunta em linguagem natural e APENAS GERA a consulta SPARQL.
    """
    try:
        ...
        if "error" in pln_result:
            return jsonify({"sparql_query": "Erro no PLN", "error": pln_result["error"]}), 500
        ...
        if "error" in query_build:
            return jsonify({"sparql_query": "Erro na montagem", "error": query_build["error"]}), 500
        
        return jsonify({"sparql_query": query_build["sparql_query"]})
    except Exception as e:
        logging.error(f"Erro em /process: {e}", exc_info=True)
        return jsonify({"error": f"Erro interno ao gerar a consulta: {e}"}), 500

@app.route('/execute_query', methods=['POST'])
def execute_query_route():
    """
    RECEBE uma string de consulta SPARQL pronta e a EXECUTA.
    """
    try:
        data = request.get_json()
        ...
        logging.info(f"Executando consulta recebida (Endpoint: {endpoint or 'Local'})")

        if endpoint:
            result = execute_remote_sparql(query_string, endpoint)
        else:
            if len(graph) == 0: raise Exception("Ontologia local não carregada.")
            result = execute_local_sparql(query_string)
...
def run_pln_processor(question: str) -> dict:
    ...
    except subprocess.CalledProcessError as e: return {"error": f"Erro no script PLN (código {e.returncode}): {e.stderr}"}
    except Exception as e: return {"error": f"Erro ao chamar PLN: {e}"}

def build_sparql_query(template_name: str, entities: dict) -> dict:
    # ... (esta função não precisa de alterações)
    if not template_name: return {"error": "Template não fornecido."}
    ...
    try:
        ...
        for key, value in entities.items():
            placeholder, formatted_value = key, ""
            if placeholder == "#ENTIDADE_NOME#" and '#ENTIDADE_NOME#@pt' in final_query:
                formatted_value = f'"{str(value).replace("\"", "\\\"")}"@pt'
