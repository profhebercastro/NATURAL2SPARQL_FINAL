...
# --- CARREGAMENTO E PREPARAÇÃO DOS DADOS (COM CAMINHOS ABSOLUTOS E ROBUSTOS) ---

# Descobre o diretório absoluto onde este script está localizado.
# Esta é a forma mais segura de garantir que os arquivos sejam encontrados.
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))

def carregar_arquivo(nome_arquivo):
    """Função auxiliar para carregar arquivos do mesmo diretório que o script."""
    caminho_completo = os.path.join(SCRIPT_DIR, nome_arquivo)
    if not os.path.exists(caminho_completo):
        # Lança um erro claro se o arquivo não for encontrado, o que ajuda na depuração.
        raise FileNotFoundError(f"Arquivo de configuração essencial não encontrado: {caminho_completo}")
    return caminho_completo

# Carrega todos os arquivos de configuração
thesaurus = json.load(open(carregar_arquivo('Thesaurus.json'), 'r', encoding='utf-8'))
empresa_map = json.load(open(carregar_arquivo('empresa_nome_map.json'), 'r', encoding='utf-8'))
setor_map = json.load(open(carregar_arquivo('setor_map.json'), 'r', encoding='utf-8'))

# Carrega as perguntas de referência
reference_templates = {}
with open(carregar_arquivo('Reference_questions.txt'), 'r', encoding='utf-8') as f:
    for line in f:
        line = line.strip()
        if line and ';' in line:
...
# --- FUNÇÕES AUXILIARES (sem alterações na lógica) ---
# ... (Nenhuma mudança necessária nas funções normalizar_pergunta, extrair_entidades, identificar_metrica) ...
