...
def extrair_entidades_e_parametros(pergunta_lower):
    ...
    # 5. Extrai Padrão Regex para tipo de ação
...
def process_question():
    ...
    # 1. Classificação do Template (Regras + Similaridade)
    pergunta_sem_acento = remover_acentos(pergunta_lower)
    template_id_final = None

    # Bloco de regras com prioridade
    if "ordinária" in pergunta_lower or "preferencial" in pergunta_lower or "unit" in pergunta_lower or "ordinaria" in pergunta_sem_acento:
        template_id_final = 'Template_5B'
    elif "código de negociação" in pergunta_lower or "codigo de negociacao" in pergunta_lower:
