...
def extrair_todas_entidades(pergunta_lower):
    entidades = {}
    # 1. Datas
    match_data = re.search(r'(\d{1,2})/(\d{1,2})/(\d{4})', pergunta_lower)
    if match_data:
        dia, mes, ano = match_data.groups()
        entidades['data'] = f"{ano}-{mes.zfill(2)}-{dia.zfill(2)}"

    # 2. Limites numéricos
    limit_match = re.search(r'\b(as|os)?\s*(\d+|cinco|tres|três|duas|dois|quatro)\s+(acoes|ações|papeis|papéis)\b', pergunta_lower, re.IGNORECASE)
    ...
    elif re.search(r'\btop\s*(\d+)\b', pergunta_lower, re.IGNORECASE):
        entidades['limite'] = re.search(r'\btop\s*(\d+)\b', pergunta_lower, re.IGNORECASE).group(1)

    # 3. Métricas (Lógica refeita para robustez)
    texto_sem_acento = remover_acentos(pergunta_lower)
    mapa_ranking = {
    ...
    # Passada 1: Encontrar a melhor correspondência para ranking
    best_rank_match = ''; rank_key = None
    for chave, sinonimos in mapa_ranking.items():
        for s in sinonimos:
            if re.search(r'\b' + remover_acentos(s) + r'\b', texto_sem_acento) and len(s) > len(best_rank_match):
                best_rank_match = s; rank_key = chave
    if rank_key:
        entidades['ranking_calculation'] = rank_key

    # Passada 2: Encontrar a melhor correspondência para a métrica de resultado, ignorando o texto do ranking
    texto_para_resultado = texto_sem_acento.replace(remover_acentos(best_rank_match), '') if best_rank_match else texto_sem_acento
    best_metric_match = ''; metric_key = None
    for chave, sinonimos in mapa_metricas.items():
        for s in sinonimos:
            if re.search(r'\b' + remover_acentos(s) + r'\b', texto_para_resultado) and len(s) > len(best_metric_match):
                best_metric_match = s; metric_key = chave
    if metric_key:
    ...
    for key, tickers in index_map.items():
        if re.search(r'\b(no|do|da|de|entre as|do indice|acoes do)?\s*' + re.escape(key.lower()) + r'\b', remover_acentos(pergunta_lower)):
            entidades['lista_tickers'] = tickers; break
    if 'lista_tickers' not in entidades:
        for key, setor in setor_map.items():
            if re.search(r'\b' + re.escape(remover_acentos(key.lower())) + r'\b', remover_acentos(pergunta_lower)):
                entidades['nome_setor'] = setor; break
    if not any(k in entidades for k in ['lista_tickers', 'nome_setor']):
        ticker_match = re.search(r'\b([A-Z]{4}[0-9]{1,2})\b', pergunta_lower.upper())
        if ticker_match:
            ...
            for key in sorted(empresa_map.keys(), key=len, reverse=True):
                if re.search(r'\b' + re.escape(key.lower()) + r'\b', remover_acentos(pergunta_lower)):
...
# --- API FLASK (lógica de decisão inalterada, mas deve funcionar com entidades corretas) ---
app = Flask(__name__)
