...
public class Ontology {
    ...
    private void deleteInferredModelCache() {
        try {
            // Tenta deletar o arquivo de cache no sistema de arquivos de produção
            ...
            // Tenta também pelo ClassPathResource, embora seja menos provável de funcionar para escrita/deleção
            ClassPathResource inferredResource = new ClassPathResource(INFERENCE_OUTPUT_FILENAME);
            if (inferredResource.isFile()) {
                Path path = Paths.get(inferredResource.getURI());
                if (Files.deleteIfExists(path)) { 
                    logger.warn("Arquivo de cache da ontologia (via ClassPath) deletado para forçar reconstrução: {}", path); 
                }
            }
        } catch (Exception e) {
            logger.warn("Não foi possível deletar o arquivo de cache da ontologia. Isso pode ser normal. Erro: {}", e.getMessage());
        }
    }
