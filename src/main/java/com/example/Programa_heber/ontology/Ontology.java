...
public class Ontology {
    ...
    private Model buildBaseModelFromSources() throws IOException {
        ...
        Map<String, String> tickerToEmpresa = loadTickerToEmpresaMap(INFO_EMPRESAS_FILE);

        for (String filePath : PREGAO_FILES) {
            loadPregaoDataAndCreateEntities(model, filePath, tickerToEmpresa);
        }
        return model;
    }

    private Map<String, String> loadTickerToEmpresaMap(String resourcePath) throws IOException {
        ...
        try (InputStream excelFile = new ClassPathResource(resourcePath).getInputStream(); Workbook workbook = new XSSFWorkbook(excelFile)) {
            ...
            for (Row row : sheet) {
                ...
                if (ticker != null && nomeEmpresa != null) {
                    map.put(ticker.trim(), nomeEmpresa.trim());
                }
            }
        }
        return map;
    ...
    private void loadPregaoDataAndCreateEntities(Model model, String resourcePath, Map<String, String> tickerToEmpresa) throws IOException {
        logger.info(">> Carregando e Criando Entidades de: {}", resourcePath);
        try (InputStream excelFile = new ClassPathResource(resourcePath).getInputStream(); Workbook workbook = new XSSFWorkbook(excelFile)) {
            Sheet sheet = workbook.getSheetAt(0);
            ...
            for (Row row : sheet) {
                ...
                if (tickerToEmpresa.containsKey(tickerClean)) {
                    String nomeEmpresa = tickerToEmpresa.get(tickerClean);
                    Resource empresaRes = model.createResource(ONT_PREFIX + normalizarParaURI(nomeEmpresa));
                    addStatement(model, empresaRes, RDF.type, model.createResource(ONT_PREFIX + "Empresa"));
                    addStatement(model, empresaRes, RDFS.label, nomeEmpresa, "pt");
                ...
                Resource negociadoResource = model.createResource(ONT_PREFIX + "Negociado_" + tickerClean + "_" + dataFmt.replace("-", ""));
                addStatement(model, negociadoResource, RDF.type, model.createResource(ONT_PREFIX + "Negociado_Em_Pregao"));
                addStatement(model, vmRes, model.createProperty(ONT_PREFIX, "negociado"), negociadoResource);
